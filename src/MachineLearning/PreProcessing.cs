...
public static class PreProcessing
{
    ...
    public static async Task<string> ExtractNYTData(string html)
    {
        var doc = new HtmlDocument();
        doc.LoadHtml(html);
        ...
        foreach (HtmlNode textNode in doc.DocumentNode.Descendants().Where(node => node.NodeType == HtmlNodeType.Element))
        {
            // only grab elements that are balanced
            if (!textNode.ParentNode.GetClasses().Contains("balancedHeadline"))
            {
                continue;
            }

            await sw.WriteLineAsync(textNode.OuterHtml);
        }
    ...
    public static async Task<string> ExtractCNNData(string html)
    {
        var doc = new HtmlDocument();
        doc.LoadHtml(html);
        ...
        {
            // only grab elements that are balanced
            if (!textNode.ParentNode.GetClasses().Contains("cd_headline-text"))
            {
                continue;
            }
